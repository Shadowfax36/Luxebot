...
class Tickets(commands.Cog):
    ...
    async def ticket_setup(self, ctx):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await db.commit()
        await ctx.send(f"Ticket category created! Now run `!ticket panel #channel` to set up the panel.")
    ...
    async def close_ticket(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="Ticket Closed", description=f"Closed by {interaction.user.mention}", color=0xE74C3C)
