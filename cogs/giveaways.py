...
class Giveaways(commands.Cog):
    ...
    async def gstart(self, ctx, duration: str, winners: str, *, prize: str):
        ...
        try:
            winner_count = int(winners.replace("w", ""))
        except ValueError:
    ...
    async def end_giveaway(self, giveaway_id, channel_id, message_id, prize, winner_count):
        ...
        try:
            ...
            if not users:
                embed = discord.Embed(title=f"GIVEAWAY ENDED: {prize}", description="No valid entrants.", color=0x95A5A6)
            ...
            embed = discord.Embed(title=f"GIVEAWAY ENDED: {prize}", description=f"**Winners:** {mentions}", color=0x95A5A6)
            await msg.edit(embed=embed)
